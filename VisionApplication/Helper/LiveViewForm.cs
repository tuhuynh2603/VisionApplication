...
using Keyence.AR.VncClient.Common;
using Org.BouncyCastle.Utilities.Net;

namespace Keyence.AutoID.SDK;

public class LiveViewForm : UserControl, IDisposable
{
    public enum ImageBinningType
    {
        None,
        OneQuarter,
        OneNinth,
    ...
    private const string CATEGORY_NAME = "LiveView Settings";

    private const int _MAXIMAGEWIDTH = 2048;

    private const int _MAXIMAGEHEIGHT = 1536;

    private const short _PORTNUMBER = 5920;

    private int _timeoutMs = 2000;

    private ImageBinningType _binningType = ImageBinningType.OneQuarter;

    private ImageFormatType _imageFormat = ImageFormatType.Jpeg;

    private int _imageQuality = 5;

    private int _pullTimeSpan = 100;

    private IContainer components;

    private VncClientControlCommon _vncClient;
    ...
    public string IpAddress { get; set; } = "192.168.100.100";
    public ImageBinningType BinningType
    {
        get
        {
            return _binningType;
        ...
        set
        {
            if (Enum.IsDefined(typeof(ImageBinningType), value))
            {
                _binningType = value;
            }
        }
    ...
    public ImageFormatType ImageFormat
    {
        ...
        set
        {
            if (Enum.IsDefined(typeof(ImageFormatType), value))
            {
                _imageFormat = value;
            }
        }
    ...
    public int PullTimeSpan
    {
        get
        {
            return _pullTimeSpan;
        }
        set
        {
            if (value >= 1 && value <= 65535)
    ...
    public LiveViewForm(string strIP)
    {
        IpAddress = strIP;
        InitializeComponent();
        _vncClient.BinningType = VncImageBinningType.OneQuarter;
        _vncClient.ImageFormat = VncImageFormat.Jpeg;
        _vncClient.ImageQuality = 5;
    }
    ...
    public bool BeginReceive()
    {
        lock (_lockEvent)
        {
            ...
            if (initializeTargetReader())
            {
                _vncClient.BinningType = (VncImageBinningType)BinningType;
                _vncClient.ImageFormat = (VncImageFormat)ImageFormat;
                _vncClient.ImageQuality = ImageQuality;
                _vncClient.Timeout = TimeoutMs;
                _vncClient.MaxImageAreaSize = new Size(2048, 1536);
                _vncClient.PortNumber = 5920;
                _vncClient.IsEnablePullMode = true;
                _vncClient.PullTimeSpan = PullTimeSpan;
                VncClientControlCommon.AutoInvalidate = true;
                _vncClient.Visible = true;
                if (_vncClient.RegisterHead())
                {
                    if (!_vncClient.Start())
                    {
                        _vncClient.UnregisterHead();
                    }
    ...
    public bool DownloadRecentImage(string dstFile)
    {
        lock (_lockEvent)
        {
            if (_disposed)
            ...
            try
            {
                _vncClient.IsEnabledGetAlternateImage = true;
                using (AlternateImage alternateImage = _vncClient.GetAlternateImage())
                {
                    using FileStream fileStream = new FileStream(dstFile, FileMode.Create, FileAccess.Write);
                    byte[] buffer = new byte[alternateImage.ImageStream.Length];
                    alternateImage.ImageStream.Read(buffer, 0, (int)alternateImage.ImageStream.Length);
                    fileStream.Write(buffer, 0, (int)alternateImage.ImageStream.Length);
                }

                _vncClient.IsEnabledGetAlternateImage = false;
            }
            catch (Exception)
            {
                return false;
    ...
    private bool initializeTargetReader()
    {
        ...
        {
            _vncClient.IPAddress = address;
            _vncClient.InitializeProtocol(VncProtocolType.Ether);
            _vncClient.ImageRectangle = new Rectangle(0, 0, 800, 600);
            return true;
        }

        if (Regex.IsMatch(IpAddress, "USBPID:(\\d)") && int.TryParse(Regex.Replace(IpAddress, "USBPID:(\\d)", "$1"), out result))
        {
            _vncClient.UsbPid = result;
            _vncClient.InitializeProtocol(VncProtocolType.USB);
            _vncClient.ImageRectangle = new Rectangle(0, 0, 800, 600);
            return true;
        }
    ...
    {
        System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Keyence.AutoID.SDK.LiveviewForm));
        this._vncClient = new Keyence.AR.VncClient.Common.VncClientControlCommon();
        base.SuspendLayout();
        this._vncClient.AutoSize = true;
        this._vncClient.BackColor = System.Drawing.Color.Black;
        this._vncClient.BinningType = Keyence.AR.VncClient.Common.VncImageBinningType.OneQuarter;
        this._vncClient.Cursor = System.Windows.Forms.Cursors.Default;
        this._vncClient.DisplayBankNumber = Keyence.AR.VncClient.Common.VncDisplayBankNumber.AllBank;
        this._vncClient.DisplayBurstNumber = Keyence.AR.VncClient.Common.VncDisplayBankNumber.AllBank;
        this._vncClient.DisplayCodeFrame = true;
        this._vncClient.DisplayDecodeResult = Keyence.AR.VncClient.Common.VncDisplayDecodeResult.AllResult;
        this._vncClient.DisplayFps = false;
        this._vncClient.Dock = System.Windows.Forms.DockStyle.Fill;
        this._vncClient.DrawRectangleOnly = false;
        this._vncClient.ImageFormat = Keyence.AR.VncClient.Common.VncImageFormat.Jpeg;
        this._vncClient.ImageQuality = 5;
        this._vncClient.ImageRectangle = new System.Drawing.Rectangle(0, 0, 5456, 3076);
        this._vncClient.ImageSize = new System.Drawing.Size(5456, 3076);
        this._vncClient.IPAddress = System.Net.IPAddress.Parse(IpAddress);//(IPAddress)("127.0.0.1"); resources.GetObject("_vncClient.IPAddress");
        this._vncClient.IsAutoUpdateImageArea = true;
        this._vncClient.IsEnableBuildInOperation = true;
        this._vncClient.IsEnabledGetAlternateImage = false;
        this._vncClient.IsEnablePullMode = false;
        this._vncClient.Location = new System.Drawing.Point(0, 0);
        this._vncClient.MaxImageAreaSize = new System.Drawing.Size(5456, 3076);
        this._vncClient.Name = "_vncClient";
        this._vncClient.PortNumber = 5910;
        this._vncClient.PullTimeSpan = 0;
        this._vncClient.Size = new System.Drawing.Size(50, 50);
        this._vncClient.TabIndex = 0;
        this._vncClient.Timeout = 2000;
        this._vncClient.UsbPid = 5;
        base.AutoScaleDimensions = new System.Drawing.SizeF(6f, 12f);
        base.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
        this.BackColor = System.Drawing.Color.Black;
        base.Controls.Add(this._vncClient);
        base.Name = "LiveViewForm";
...
Load from: 'C:\Program Files\dotnet\packs\Microsoft.NETCore.App.Ref\8.0.8\ref\net8.0\System.dll'
------------------
Resolve: 'VncClientControlCommon, Version=6.49.0.0, Culture=neutral, PublicKeyToken=null'
Found single assembly: 'VncClientControlCommon, Version=6.49.0.0, Culture=neutral, PublicKeyToken=null'
Load from: 'C:\Wisely\C#\VisionApplication\VisionApplication\VncClientControlCommon.dll'
------------------
Resolve: 'VncClientControlCommonLib, Version=6.49.0.0, Culture=neutral, PublicKeyToken=null'
Found single assembly: 'VncClientControlCommonLib, Version=6.49.0.0, Culture=neutral, PublicKeyToken=null'
Load from: 'C:\Wisely\C#\VisionApplication\VisionApplication\VncClientControlCommonLib.dll'
------------------
Resolve: 'Communication, Version=6.49.0.0, Culture=neutral, PublicKeyToken=null'
Found single assembly: 'Communication, Version=6.49.0.0, Culture=neutral, PublicKeyToken=null'
Load from: 'C:\Wisely\C#\VisionApplication\VisionApplication\Communication.dll'
------------------
...
Found single assembly: 'System.IO.Ports, Version=8.0.0.0, Culture=neutral, PublicKeyToken=cc7b13ffcd2ddd51'
WARN: Version mismatch. Expected: '0.0.0.0', Got: '8.0.0.0'
Load from: 'C:\Users\huynh\.nuget\packages\system.io.ports\8.0.0\lib\net8.0\System.IO.Ports.dll'
------------------
